...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            next_url = request.GET.get('next', 'form_list')
...
def user_register(request):
    if request.method == 'POST':
        ...
        if password1 != password2:
            ...
            return render(request, 'forms_app/register.html')
...
def form_view(request, form_id):
    ...
    if form.created_by == request.user:
        return render(request, 'forms_app/form_view.html', {'form': form})
    # For other users, redirect to form submission
    return redirect('form_submit', form_id=form_id)
...
def form_analytics(request, form_id):
    ...
    if form.created_by != request.user:
        ...
        return redirect('form_list')
    return render(request, 'forms_app/form_analytics.html', {'form': form})
...
def form_submit(request, form_id):
    ...
    return render(request, 'forms_app/form_submit.html', {'form': form})
